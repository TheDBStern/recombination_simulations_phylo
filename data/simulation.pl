...
`ms $total $loci -T -r 0.001 1000 -I 8 $ind $ind $ind $ind $ind $ind $ind $ind -ej $nodeT0 $nodeN0 $nodeO0 -ej $nodeT1 $nodeN1 $nodeO1 -ej $nodeT2 $nodeN2 $nodeO2  -ej $nodeT3 $nodeN3 $nodeO3 -ej $nodeT4 $nodeN4 $nodeO4 -ej $nodeT5 $nodeN5 $nodeO5 -ej $nodeT6 $nodeN6 $nodeO6 | tail +4 | grep -v // >$dir/$genealogy/$genealogy_file`;
}
}
sub speciestree {
		undef @nodeT;
...
sub seqgene {
	...
	`seq-gen -mHKY -l1000 -n$rep -z $randseed -s $rate -p 50 -a0.8 -g4 -f0.3 0.2 0.3 0.2 -t3.0 -on <$dir/$genealogy/$genealogy_file >$dir/$genealogy/$sequence_file`; ####
